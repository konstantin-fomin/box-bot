...
`GEMINI_API_KEY` опционален: без него бот запускается, но свободные текстовые AI-команды и голосовое распознавание отключены.
...
- `⚙️ Ещё` → `👥 Моя группа`
- `где блендер?`
- голосом или обычным сообщением: `создай коробку кухня: тарелки, кружки`
- голосом или обычным сообщением: `добавь в коробку KUHN-01 чайник`
...
AI-команды, которые меняют данные (создание коробки, добавление вещей, смена статуса, удаление), сначала показываются на подтверждение inline-кнопками. Поиск выполняется сразу.
